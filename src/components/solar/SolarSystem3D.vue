...
let sunHalo: THREE.Sprite | null = null
...
let orbitClock = 0
let focusedId: PlanetId | null = null
let focusDistanceMultiplier = 1
let manualCameraOverrideUntil = 0
let lastFrameTime = 0
let lastLabelUpdate = 0
let lastPointerWasDrag = false

const focusWorldPosition = new THREE.Vector3()
const focusCameraOffset = new THREE.Vector3(1.6, 1.15, 2.35)
...
function configFor(planet: Planet): PlanetConfig {
  const motion = planet.motion
  const sizeBoost = props.mode === 'hero' ? 1.45 : props.mode === 'map' ? 1 : 2.2

  return {
    orbitRadius: (props.scaleMode === 'real' ? motion.realOrbitRadius : motion.orbitRadius),
    orbitSpeed: motion.orbitSpeed,
    rotationSpeed: motion.rotationSpeed,
    size: motion.radius * sizeBoost,
    tilt: motion.axialTilt,
    startAngle: motion.initialAngle
  }
}
  ...
  const texture = textureLoader.load(
    ...
    () => {
      if (planetMaterial) {
        ...
        planetMaterial.color.set(planet.color)
        planetMaterial.needsUpdate = true
      }
    }
  )
...
function setCameraPreset() {
  if (!camera) return
  if (props.mode === 'hero') {
    camera.position.set(-1.4, 10.5, 23)
...
function buildScene() {
  ...
  planets.forEach((planet, index) => {
    ...
    if (props.mode !== 'detail' && orbitGroup) {
      const orbit = createOrbit(configFor(planet).orbitRadius, index)
...
function spawnParticleBurst(position: THREE.Vector3, type: 'pinch' | 'fist' | 'focus' = 'pinch', attachedTo?: PlanetId | null) {
  if (!scene) return
  const count = isMobile() ? (type === 'fist' ? 30 : 18) : type === 'fist' ? 72 : type === 'focus' ? 48 : 28
  const positions = new Float32Array(count * 3)
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const speed = type === 'fist' ? 0.022 + Math.random() * 0.045 : 0.012 + Math.random() * 0.024
  ...
  const material = new THREE.PointsMaterial({
    size: type === 'fist' ? 0.045 : 0.035,
    vertexColors: true,
    transparent: true,
    opacity: type === 'fist' ? 0.82 : 0.7,
    depthWrite: false,
    blending: THREE.AdditiveBlending
  })
  ...
  const shockwave = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({
      color: type === 'fist' ? 0xfacc15 : 0x22d3ee,
      transparent: true,
      opacity: 0.42,
      side: THREE.DoubleSide,
      depthWrite: false,
  ...
  const flashTexture = makeHalo('rgba(255,255,255,0.96)', 96)
  ...
  particleBursts.push({
    ...
    velocities,
    age: 0,
    duration: type === 'fist' ? 0.95 : 0.62,
    opacity: type === 'fist' ? 0.82 : 0.7,
    attachedTo: attachedTo || undefined,
    anchor: attachedTo ? position.clone() : null
  })
}
...
function handlePointerDown(event: PointerEvent) {
  updatePointer(event)
  lastPointerWasDrag = false
  manualCameraOverrideUntil = performance.now() + 8000
...
function handleWheel(event: WheelEvent) {
  ...
  manualCameraOverrideUntil = performance.now() + 8000
  if (focusedId) {
    focusDistanceMultiplier = THREE.MathUtils.clamp(focusDistanceMultiplier + event.deltaY * 0.004, 0.55, 2.4)
  } else {
...
function focusPlanet(id: PlanetId) {
  ...
  updateFocusTargets()
  if (changed) spawnParticleBurst(focusWorldPosition, 'focus', id)
...
function resetFocusVisuals() {
  planetMeshes.forEach(item => {
    const material = item.material as THREE.MeshStandardMaterial
    material.emissiveIntensity = props.mode === 'hero' ? 0.18 : 0.05
  })
...
function updateFocusTargets() {
  if (!focusedId) return
  const mesh = planetMeshes.get(focusedId)
  if (!mesh) return
  mesh.getWorldPosition(focusWorldPosition)
  targetLookAt.copy(focusWorldPosition)
  targetCameraPosition.copy(focusWorldPosition).addScaledVector(focusCameraOffset, focusDistanceMultiplier)
}
...
function gestureZoom(delta: number) {
  manualCameraOverrideUntil = performance.now() + 8000
  if (focusedId) {
    focusDistanceMultiplier = THREE.MathUtils.clamp(focusDistanceMultiplier - delta * 0.002, 0.55, 2.4)
    return
  }
  if (props.mode === 'detail') {
...
function updateParticleBursts(delta: number) {
  for (let index = particleBursts.length - 1; index >= 0; index -= 1) {
    ...
    if (burst.attachedTo && burst.anchor) {
      const attachedMesh = planetMeshes.get(burst.attachedTo)
      if (attachedMesh) {
        attachedMesh.getWorldPosition(focusWorldPosition)
        const dx = focusWorldPosition.x - burst.anchor.x
        const dy = focusWorldPosition.y - burst.anchor.y
        const dz = focusWorldPosition.z - burst.anchor.z
        for (let pointIndex = 0; pointIndex < burst.positions.length / 3; pointIndex += 1) {
          const offset = pointIndex * 3
          burst.positions[offset] += dx
          burst.positions[offset + 1] += dy
          burst.positions[offset + 2] += dz
        }
        burst.anchor.copy(focusWorldPosition)
        burst.shockwave.position.copy(focusWorldPosition)
        if (burst.flash) burst.flash.position.copy(focusWorldPosition)
      }
    }
    ...
    burst.material.opacity = Math.max(0, burst.opacity * (1 - progress))
    const waveScale = 0.25 + progress * 0.95
    burst.shockwave.scale.set(waveScale, waveScale, waveScale)
    const waveMaterial = burst.shockwave.material as THREE.MeshBasicMaterial
    waveMaterial.opacity = Math.max(0, 0.42 - progress * 0.42)
    if (burst.flash) {
      const flashScale = 0.4 + (1 - progress) * 0.8
      burst.flash.scale.set(flashScale, flashScale, 1)
      ;(burst.flash.material as THREE.SpriteMaterial).opacity = Math.max(0, 0.5 - progress * 0.5)
    }
    ...
    if (progress >= 1) {
      ...
      if (burst.flash) scene?.remove(burst.flash)
      burst.geometry.dispose()
      burst.material.dispose()
      burst.shockwave.geometry.dispose()
      ;(burst.shockwave.material as THREE.Material).dispose()
      if (burst.flash) {
...
function updateLabels() {
  ...
  if (now - lastLabelUpdate < 66) return
  lastLabelUpdate = now
  ...
  planetMeshes.forEach((mesh, id) => {
    ...
    projectedPosition.y += props.mode === 'detail' ? 1.85 : 0.48
    const projected = projectedPosition.project(camera!)
...
function animate() {
  ...
  lastFrameTime = now
  const timeSpeed = props.paused ? 0 : props.speedMultiplier * motionScale()
  orbitClock += delta * timeSpeed

  starField && (starField.rotation.y += delta * 0.025 * motionScale())
  asteroidBelt && (asteroidBelt.rotation.y += delta * 0.06 * motionScale())
  if (sunMesh) sunMesh.rotation.y += delta * 0.08 * motionScale()
  if (sunHalo) {
    sunHalo.scale.multiplyScalar(1 + Math.sin(orbitClock * 1.8) * 0.0007)
    sunHalo.material.opacity = 0.86 + Math.sin(orbitClock * 1.6) * 0.08
  }

  if (props.autoCruise && props.mode === 'map' && !dragStart && now > manualCameraOverrideUntil) {
    solarRoot.rotation.y += delta * 0.11
  }
  ...
  planetMeshes.forEach((mesh, id) => {
    ...
    mesh.scale.lerp(scaleTarget, 0.08)
...
function cleanupScene(disconnectObserver = true) {
  cancelAnimationFrame(animationId)
  if (disconnectObserver) resizeObserver?.disconnect()
  renderer?.dispose()
  ...
  scene?.traverse(object => {
    ...
    materials.forEach(item => {
      const textured = item as THREE.Material & { map?: THREE.Texture | null; alphaMap?: THREE.Texture | null }
...
<template>
  ...
  >
    ...
    <div v-if="webglError" class="webgl-fallback scan-panel">
      <strong>当前设备无法启动 3D 场景</strong>
      <span>请启用硬件加速，或使用支持 WebGL 的现代浏览器。</span>
    </div>
  </div>
</template>
